...
class FormTesting(TestCase):
    ...
    def test_forms(self):
        ...
        self.assertTrue(UserProfileForm(
            data={'website': "test@google.com", 'company': "Amazon", 'about': "Hey there, nice weather we're having?", 'searchinginfo': "Lorum Ipsum", 'isEmployer': False, }).is_valid())  # Should be valid because all inputs are valid.
...
class LinkedOn_TemplatesStructureTests(TestCase):
    """
    are templates, static and media files set up correctly
    """
    ...
    def test_template_dir_setting(self):
        """
        Does the TEMPLATE_DIR setting exist in the right directory?
        """
        variable_exists = 'TEMPLATE_DIR' in dir(settings)
        self.assertTrue(variable_exists, f"{FAILURE_HEADER}TEMPLATE_DIR not defined.{FAILURE_FOOTER}")

        template_dir_value = os.path.normpath(settings.TEMPLATE_DIR)
        template_dir_computed = os.path.normpath(self.templates_dir)
        self.assertEqual(template_dir_value, template_dir_computed, f"{FAILURE_HEADER}TEMPLATE_DIR not expected path.{FAILURE_FOOTER}")
    ...
    def test_template_lookup_path(self):
        ...
        self.assertTrue(found_path, f"{FAILURE_HEADER}not in template dir lookup{FAILURE_FOOTER}")
